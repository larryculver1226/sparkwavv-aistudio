...
# Accept build arguments for CLIENT-SIDE variables only
ARG VITE_FIREBASE_API_KEY
ARG VITE_FIREBASE_PROJECT_ID
ARG APP_URL

# Set them as environment variables for the build process (Vite needs these)
ENV VITE_FIREBASE_API_KEY=$VITE_FIREBASE_API_KEY
ENV VITE_FIREBASE_PROJECT_ID=$VITE_FIREBASE_PROJECT_ID
ENV APP_URL=$APP_URL

# RUN npm run build which compiles both vite assets and server.ts to dist/
RUN npm run build
