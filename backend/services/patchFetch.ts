...
if (typeof global !== 'undefined' && typeof window === 'undefined') {
  ...
  if (!(global as any)[FETCH_PATCHED]) {
    ...
    const sharedUrl = process.env.SHARED_APP_URL || "https://ais-pre-6de5lrtpnvciah3xwxmagf-232918548667.us-east1.run.app";
    ...
    if (typeof originalFetch === 'function') {
      (global as any).fetch = async function(input: any, init: any = {}) {
        ...
        if (urlStr && urlStr.includes('googleapis.com')) {
           ...
           // Attempt rotation: Shared -> Dev -> None
           let response = await tryFetch(sharedUrl);
           if (response.status === 403) {
             console.warn(`[Global Fetch Patch] 403 with ${sharedUrl}, retrying with ${appUrl}...`);
             response = await tryFetch(appUrl);
           }
           if (response.status === 403) {
             console.warn(`[Global Fetch Patch] 403 with ${appUrl}, retrying without referer...`);
