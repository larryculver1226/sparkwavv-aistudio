...
const PERSONAS = [
  {
    ...
    description: 'Sharp, data-driven, and looks for flaws in the narrative.',
    icon: ShieldCheck,
    voice: 'Charon',
  },
  {
    ...
    description: 'Passionate, looks for cultural alignment and big-picture thinking.',
    icon: Zap,
    voice: 'Zephyr',
  },
  {
    id: 'pragmatic_manager',
    name: 'The Pragmatic Manager',
    description: 'Focused on execution, technical skills, and team fit.',
    icon: Target,
    voice: 'Kore',
  },
];
      ...
      <div className="p-8 max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          ...
          <div className="glass-panel p-8 rounded-[2rem] border border-white/5 bg-black/40">
            ...
            <p className="text-sm text-white/80 leading-relaxed mb-8">{debrief.overallVerdict}</p>
            <div className="space-y-3">
              ...
              {debrief.tacticalAdvice.map((advice: string, i: number) => (
                <div
                  key={i}
                  className="flex items-start gap-3 p-3 rounded-xl bg-white/5 border border-white/5 text-[11px] text-white/60"
                >
        ...
        <div className="flex-1 flex flex-col h-full p-12 relative z-10">
            ...
            <div className="flex items-center gap-6">
              <div className="w-16 h-16 rounded-2xl bg-neon-cyan/10 border border-neon-cyan/20 flex items-center justify-center">
                {PERSONAS.find((p) => p.id === activePersona)?.icon &&
                  React.createElement(PERSONAS.find((p) => p.id === activePersona)!.icon, {
          ...
          >
            {messages.map((msg, i) => (
              ...
              >
                <div
                  className={`max-w-[70%] ${msg.role === 'skylar' ? 'bg-white/5 border border-white/10' : 'bg-neon-cyan/10 border border-neon-cyan/20'} p-6 rounded-3xl relative`}
                >
                  {msg.role === 'skylar' && (
