...
export const EveningSpark: React.FC<EveningSparkProps> = ({ energyTrough, onClose }) => {
  ...
  const [currentPhase, setCurrentPhase] = useState<'relax' | 'refresh' | 'review' | 'reflect'>(
    'relax'
  );
  ...
  const startSession = async () => {
    const text =
      "Hello! It's your scheduled Energy Trough. I'm Skylar, your Soft Coach. Instead of pushing through, let's take a moment to reboot. We'll start with a quick relaxation exercise.";
    try {
  ...
  const handlePhaseChange = async (phase: typeof currentPhase) => {
    ...
    const texts = {
      relax:
        'Close your eyes and focus on your breath. Let the tension in your shoulders melt away.',
      refresh: 'Take a quick walk or stretch. Move your body to reinvigorate your mind.',
      review:
        "Let's look at your progress today. You've made significant strides in your Discovery phase.",
        ...
        <div className="relative z-10 text-center space-y-12 max-w-2xl px-8">
          <div className="space-y-4">
            ...
            <h2 className="text-4xl font-display font-bold text-white tracking-tight">
              Evening Spark Session
            </h2>
            <p className="text-neon-cyan/60 font-bold uppercase tracking-[0.2em] text-xs">
              Energy Trough Reboot Active
            </p>
          </div>
