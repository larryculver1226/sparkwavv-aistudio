...
import { Brain, TrendingUp, Shield, Zap, DollarSign, Heart, Lightbulb, Users, BookOpen } from 'lucide-react';
  ...
  return (
    <section className="py-24 px-6 max-w-7xl mx-auto">
      <div className="text-center mb-16 space-y-4">
        <h2 className="text-4xl md:text-5xl font-display font-bold">The Dual-Core Value Proposition</h2>
        <p className="text-white/60 max-w-2xl mx-auto">
          The Sparkwavv experience is engineered to address the distinct needs of both hemispheres of the professional mind.
        </p>
      </div>
        ...
        >
          <div className="flex items-center gap-4 border-b border-white/10 pb-6">
            ...
            <div>
              ...
              <p className="text-neon-cyan text-sm uppercase tracking-widest">Quantitative Benefits</p>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="space-y-4">
              <div className="flex justify-between items-end">
                ...
                <span className="text-2xl font-bold text-neon-cyan">${salary.toLocaleString()}</span>
              </div>
              <input 
                type="range" 
                min="50000" 
        ...
        >
          <div className="flex items-center gap-4 border-b border-white/10 pb-6">
            ...
            <div>
              ...
              <p className="text-purple-400 text-sm uppercase tracking-widest">Qualitative Benefits</p>
            </div>
          </div>
...
const ROICard = ({ icon, label, value, description }: { icon: React.ReactNode, label: string, value: number, description: string }) => (
  <div className="flex items-center justify-between p-4 rounded-xl bg-white/5 border border-white/10 hover:border-neon-cyan/30 transition-all group">
    ...
    <span className="text-lg font-mono font-bold text-neon-cyan">+${Math.round(value).toLocaleString()}</span>
  </div>
);

const QualitativeCard = ({ icon, label, text }: { icon: React.ReactNode, label: string, text: string }) => (
  <div className="flex gap-4 group">
    ...
    <div className="space-y-1">
      <h4 className="font-bold text-white group-hover:text-purple-400 transition-colors">{label}</h4>
