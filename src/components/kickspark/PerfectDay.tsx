    ...
    <div className="space-y-6">
        ...
        {schedule.map((item, i) => {
          ...
          return (
            <div key={i} className="relative flex items-start gap-6 group">
              <div
                className={`z-10 w-10 h-10 rounded-xl flex items-center justify-center border transition-all ${TYPE_COLORS[item.type]}`}
              >
      ...
      <div className="p-4 rounded-2xl bg-neon-cyan/5 border border-neon-cyan/10">
        ...
        <p className="text-xs text-white/60 leading-relaxed italic">
          "Your schedule shows a high concentration of Deep Work in the morning. Consider moving
          your 'Reboot' session to 14:00 to combat your natural energy trough."
        </p>
      </div>
    </div>
  );
