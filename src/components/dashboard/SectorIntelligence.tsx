...
export const SectorIntelligence: React.FC<SectorIntelligenceProps> = ({ sector, userId }) => {
  ...
  const getSectorIcon = () => {
    switch (sector) {
      ...
      default:
        return <Zap className="w-6 h-6 text-neon-cyan" />;
    }
  };
  ...
  const getModelName = () => {
    switch (sector) {
      case 'Healthcare':
        return 'MedLM (Vertex AI)';
      case 'Legal':
        return 'SecLM (Vertex AI)';
      default:
          ...
          {!insight ? (
            <div className="space-y-4">
              <p className="text-sm text-white/60 leading-relaxed">
                As a professional in the <span className="text-white font-bold">{sector}</span>{' '}
                sector, you have access to specialized Vertex AI models for deep industry insights.
              </p>
