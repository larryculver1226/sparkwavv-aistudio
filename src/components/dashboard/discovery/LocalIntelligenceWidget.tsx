      ...
      if (chunks) {
        const mappedResults: LocalEvent[] = chunks
          ...
          .map((chunk: any) => ({
            ...
            url: chunk.maps.uri,
            type: chunk.maps.title.toLowerCase().includes('event') || chunk.maps.title.toLowerCase().includes('meetup') ? 'event' : 'location'
          }));
    ...
    <div className="glass-panel p-6 h-full flex flex-col gap-6 relative overflow-hidden group">
      ...
      <header className="flex items-center justify-between relative z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-white uppercase tracking-widest">Local Intelligence</h3>
            <p className="text-[10px] text-white/40 font-mono uppercase">Grounding Advice in Reality</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar relative z-10">
        {loading ? (
          <div className="h-full flex flex-col items-center justify-center gap-3 py-12">
            ...
            <p className="text-[10px] text-white/40 uppercase font-bold tracking-widest animate-pulse">Triangulating Events...</p>
          </div>
        ) : error ? (
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className={`text-[8px] font-bold uppercase px-1.5 py-0.5 rounded border ${
                      res.type === 'event' ? 'bg-neon-cyan/10 border-neon-cyan/30 text-neon-cyan' : 'bg-white/10 border-white/20 text-white/40'
                    }`}>
                      {res.type}
                    </span>
                    <h4 className="text-xs font-bold text-white group-hover/item:text-neon-cyan transition-colors line-clamp-1">{res.title}</h4>
                  </div>
