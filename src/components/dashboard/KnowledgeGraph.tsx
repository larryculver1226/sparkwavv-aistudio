  ...
  useEffect(() => {
    ...
    const node = g.append('g')
      ...
      .on('mouseover', function(event, d: any) {
        d3.select(this).select('circle').attr('r', d.type === 'spark' ? 15 : 11);
        ...
        link.attr('stroke-opacity', (l: any) => (l.source.id === d.id || l.target.id === d.id) ? 1 : 0.1)
            .attr('stroke', (l: any) => (l.source.id === d.id || l.target.id === d.id) ? '#00f2ff' : '#333');
      })
      .on('mouseout', function(event, d: any) {
        d3.select(this).select('circle').attr('r', d.type === 'spark' ? 12 : 8);
        d3.select(this).select('text').attr('font-size', '10px').attr('fill', '#fff');
        link.attr('stroke-opacity', 0.4).attr('stroke', (d: any) => d.type === 'dependency' ? '#ff00ff' : '#333');
      });
