...
export const NeuralSynthesisEngine: React.FC<NeuralSynthesisEngineProps> = ({ 
  ...
  const performInitialSynthesis = async () => {
    ...
    const initialGraph: GraphData = {
      nodes: [
        { id: 'spark-1', label: 'The Spark', type: 'spark', strength: 1, description: 'Core professional driver' },
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      addLog(`Tokenizing Document Structure (${file.name.split('.').pop()?.toUpperCase()})...`, 'info', 'tokenize');
      await new Promise(r => setTimeout(r, 1000));
      ...
      <div className="glass-panel p-8 border-white/10 relative overflow-hidden group min-h-[400px] flex flex-col items-center justify-center text-center space-y-6">
        ...
        <div className="relative z-10 space-y-4">
          ...
          <p className="text-sm text-white/60 max-w-sm mx-auto leading-relaxed">
            The Neural Synthesis Engine is currently locked. Complete the Ignition phase and upgrade to Discovery to initialize your career knowledge graph.
          </p>
    ...
    <div className="glass-panel border-neon-cyan/20 flex flex-col min-h-[500px] overflow-hidden">
      ...
      <div className="p-6 border-b border-white/10 flex items-center justify-between bg-white/[0.02]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-white uppercase tracking-widest">Neural Synthesis Engine</h3>
            <div className="flex items-center gap-2 mt-1">
              <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
              <span className="text-[10px] text-white/40 font-mono uppercase tracking-widest">System Online // Analytical Mode</span>
            </div>
          </div>
        </div>
          ...
          {activeTab === 'graph' ? (
            ...
            <div className="w-full h-full bg-black/60 rounded-2xl border border-white/5 p-6 font-mono overflow-hidden flex flex-col">
              <div className="flex items-center gap-2 text-neon-cyan mb-4">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest">Synthesis Processing Log</span>
              </div>
                ...
                {processingLogs.map((log, i) => (
                  <div key={i} className="flex gap-4 text-[10px] leading-relaxed">
                    ...
                    <span className={
                      ...
                      {log.status === 'success' ? '✓' : log.status === 'error' ? '✗' : '>'} {log.message}
                    </span>
                  </div>
                ))}
                {isIngesting && (
          ...
          <section className="space-y-4">
            ...
            <div 
              ...
              className={`p-6 rounded-2xl border-2 border-dashed transition-all cursor-pointer text-center space-y-3 ${
                isIngesting ? 'border-neon-cyan/50 bg-neon-cyan/5' : 'border-white/10 hover:border-neon-cyan/40 hover:bg-white/5'
              }`}
            >
              <input 
                type="file" 
            ...
            <div className="space-y-3">
              <div className="p-4 rounded-xl bg-white/5 border border-white/10 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-white/40 uppercase font-bold">Synthesis Depth</span>
              ...
              <div className="p-4 rounded-xl bg-white/5 border border-white/10 space-y-2">
                <p className="text-[10px] text-white/60 leading-relaxed italic">
                  "Skylar has identified a strong semantic bridge between your technical skills and your leadership goals."
                </p>
              </div>
            </div>
          </section>
          ...
          >
            <div className="p-8 flex items-center justify-between border-b border-white/10">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/40 uppercase tracking-[0.2em] font-mono">Full-Scale Identity Mapping v4.2</p>
                </div>
              </div>
              <button 
            ...
            <div className="flex-1 relative overflow-hidden">
              ...
              <div className="absolute bottom-12 left-12 p-8 glass-panel border-white/10 space-y-6">
                <h4 className="text-sm font-bold uppercase tracking-widest text-neon-cyan">Identity Legend</h4>
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    <div className="w-4 h-4 rounded-full bg-[#00f2ff] shadow-[0_0_10px_rgba(0,242,255,0.5)]" />
                    <span className="text-xs font-mono text-white/80">SKILLS // Technical & Soft Competencies</span>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="w-4 h-4 rounded-full bg-[#ff00ff] shadow-[0_0_10px_rgba(255,0,255,0.5)]" />
                    <span className="text-xs font-mono text-white/80">GOALS // Trajectories & Ambitions</span>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="w-4 h-4 rounded-full bg-[#00ffcc] shadow-[0_0_10px_rgba(0,255,204,0.5)]" />
                    <span className="text-xs font-mono text-white/80">VALUES // Core Drivers & Principles</span>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="w-6 h-6 rounded-full bg-white shadow-[0_0_15px_rgba(255,255,255,0.5)]" />
                    <span className="text-xs font-mono text-white/80">THE SPARK // Foundational Anchor</span>
                  </div>
                </div>
              </div>
