  ...
  return (
    <div className="space-y-8">
          ...
          {assets.map((asset) => (
            ...
            >
              <div className="flex items-start justify-between mb-6">
                <div
                  className={`w-12 h-12 rounded-2xl flex items-center justify-center ${asset.type === 'resume' ? 'bg-green-400/10 text-green-400' : 'bg-blue-400/10 text-blue-400'}`}
                >
                  {asset.type === 'resume' ? (
                ...
                <div className="flex items-center gap-2">
                  ...
                  {asset.isLocked && (
                    <div
                      className="w-6 h-6 rounded-full bg-green-400/20 flex items-center justify-center"
              ...
              {/* Background Glow */}
              <div
                className={`absolute -bottom-12 -right-12 w-24 h-24 blur-[40px] opacity-20 transition-opacity group-hover:opacity-40 ${asset.type === 'resume' ? 'bg-green-400' : 'bg-blue-400'}`}
              />
            </motion.div>
          ))}
        </AnimatePresence>
      ...
      {selectedAsset && showShareModal && (
        <ShareModal asset={selectedAsset} onClose={() => setShowShareModal(false)} />
      )}
    </div>
  );
};
