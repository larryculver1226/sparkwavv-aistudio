  ...
  for (const { name, value } of keys) {
    if (value && typeof value === 'string') {
      ...
      if (trimmed && trimmed.startsWith('AIza')) {
        ...
        console.log(`[AIConfig] SUCCESS: Found valid key in ${name}: ${masked} (length: ${trimmed.length})`);
        return trimmed;
      } else if (trimmed) {
        console.warn(`[AIConfig] WARNING: Found value in ${name} but it doesn't start with AIza: ${trimmed.substring(0, 10)}...`);
      }
    }
  }
