        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          >
            <div className="flex items-center gap-4">
              <div
                className={`w-12 h-12 rounded-2xl flex items-center justify-center ${isFirebaseConfigured ? 'bg-neon-lime/10 text-neon-lime' : 'bg-neon-magenta/10 text-neon-magenta'}`}
              >
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center p-4 rounded-xl bg-white/5 border border-white/10">
                ...
                <span
                  className={`text-xs font-bold px-2 py-1 rounded-full ${isFirebaseConfigured ? 'bg-neon-lime/20 text-neon-lime' : 'bg-neon-magenta/20 text-neon-magenta'}`}
                >
              ...
              {!isFirebaseConfigured && (
                <div className="p-4 rounded-xl bg-neon-magenta/10 border border-neon-magenta/20 text-xs text-neon-magenta leading-relaxed">
                  <strong>Error:</strong> auth/invalid-api-key. Please ensure VITE_FIREBASE_API_KEY
          ...
          >
            <div className="flex items-center gap-4">
              <div
                className={`w-12 h-12 rounded-2xl flex items-center justify-center ${status?.admin ? 'bg-neon-lime/10 text-neon-lime' : 'bg-neon-cyan/10 text-neon-cyan'}`}
              >
                {status?.admin ? (
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center p-4 rounded-xl bg-white/5 border border-white/10">
                ...
                <span
                  className={`text-xs font-bold px-2 py-1 rounded-full ${status?.admin ? 'bg-neon-lime/20 text-neon-lime' : 'bg-neon-cyan/20 text-neon-cyan'}`}
                >
              ...
              <div className="flex justify-between items-center p-4 rounded-xl bg-white/5 border border-white/10">
                ...
                <span
                  className={`text-xs font-bold px-2 py-1 rounded-full ${status?.firestore === 'connected' ? 'bg-neon-lime/20 text-neon-lime' : 'bg-neon-magenta/20 text-neon-magenta'}`}
                >
          ...
          <div className="bg-black/40 rounded-xl p-6 font-mono text-sm text-white/60 space-y-2 overflow-x-auto">
            ...
            <p>
              {status?.admin
                ...
                : '[WARN] Firebase Admin credentials missing.'}
            </p>
