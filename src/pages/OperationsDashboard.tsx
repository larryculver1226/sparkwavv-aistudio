...
export const OperationsDashboard: React.FC<{ onLogout?: () => void }> = ({ onLogout }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let endpoint = '';
      if (activeTab === 'users')
        endpoint = editingItem ? '/api/admin/update-user' : '/api/admin/create-user';
            ...
            <div className="p-8 space-y-8">
              ...
              <div className="glass-panel rounded-3xl border border-white/5 bg-white/[0.02] overflow-hidden">
                <div className="p-6 border-b border-white/5 flex items-center justify-between">
                  ...
                  <button className="text-xs font-bold text-neon-cyan uppercase tracking-widest hover:underline">
                    View All Reports
                  </button>
                </div>
                  ...
                  {[
                    {
                      ...
                      user: 'User_882',
                      content:
                        'This program is not what I expected, very disappointed with the support.',
                      reason: 'Negative Sentiment (High)',
                      time: '2m ago',
                    },
                    {
                      id: 2,
                      user: 'User_129',
                      content: 'Check out this external link for better resources: [redacted]',
                      reason: 'External Link Policy',
                      time: '15m ago',
                    },
                    {
                      id: 3,
                      user: 'User_441',
                      content: 'I hate how this cohort is structured, it makes no sense.',
                      reason: 'Hostility Detected',
                      time: '1h ago',
                    },
                  ].map((item) => (
                    <div
                      key={item.id}
                      className="p-6 flex items-start justify-between hover:bg-white/[0.01] transition-colors"
                    >
                      <div className="space-y-2">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="px-2 py-0.5 rounded bg-neon-magenta/10 text-neon-magenta text-[9px] font-bold uppercase tracking-widest border border-neon-magenta/20">
                            {item.reason}
                          </span>
                        </div>
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          className="p-2 rounded-lg bg-white/5 border border-white/10 hover:bg-neon-lime/10 hover:text-neon-lime transition-all"
                          title="Dismiss"
                        >
                          <CheckCircle2 className="w-4 h-4" />
                        </button>
                        <button
                          className="p-2 rounded-lg bg-white/5 border border-white/10 hover:bg-neon-magenta/10 hover:text-neon-magenta transition-all"
            ...
            <div className="p-8 space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[
                  ...
                  <div
                    key={i}
                    className="glass-panel p-6 rounded-3xl border border-white/5 bg-white/[0.02] relative overflow-hidden group"
                  >
                    <div className="relative z-10 space-y-4">
                      ...
                      <div className="flex items-end justify-between">
                        ...
                        <span
                          className={`text-[10px] font-bold px-2 py-1 rounded-lg bg-white/5 ${stat.trend.startsWith('+') ? 'text-neon-lime' : 'text-neon-magenta'}`}
                        >
                          {stat.trend}
                        </span>
                      </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-white/[0.02]">
                    {activeTab === 'users' && (
                      <>
                        <th className="px-8 py-5 text-[10px] font-bold text-white/30 uppercase tracking-[0.2em]">
                          User Identity
                        </th>
                        <th className="px-8 py-5 text-[10px] font-bold text-white/30 uppercase tracking-[0.2em]">
                          Status
                    ...
                    <th className="px-8 py-5 text-[10px] font-bold text-white/30 uppercase tracking-[0.2em] text-right">
                      Actions
                    </th>
                  </tr>
                </thead>
                  ...
                  {loading ? (
                    <tr>
                      <td colSpan={10} className="px-8 py-24 text-center">
                        ...
                        <p className="text-white/40 font-display font-medium uppercase tracking-widest text-sm">
                          Synchronizing Data...
                        </p>
                      </td>
                    </tr>
                  ) : (activeTab === 'users' ? users : activeTab === 'cohorts' ? cohorts : programs)
                      .length === 0 ? (
                    <tr>
                      <td colSpan={10} className="px-8 py-24 text-center">
                        <p className="text-white/20 font-display font-medium uppercase tracking-widest text-sm">
                          ...
                          <>
                            <td className="px-8 py-6">
                              <div className="flex items-center gap-4">
                                ...
                                <div>
                                  <p className="font-display font-bold text-white group-hover:text-neon-cyan transition-colors">
                            ...
                            <td className="px-8 py-6">
                              <span
                                className={`inline-flex items-center gap-2 px-3 py-1 rounded-xl text-[10px] font-bold uppercase tracking-widest ${
                                  item.disabled
                              ...
                              >
                                <div
                                  className={`w-1.5 h-1.5 rounded-full ${item.disabled ? 'bg-neon-magenta' : 'bg-neon-lime'} animate-pulse`}
                                />
                            ...
                            <td className="px-8 py-6">
                              <span className="text-xs font-bold text-neon-cyan uppercase tracking-widest">
                                {item.subscriptionTier || 'DIVE-IN'}
                              </span>
                            </td>
                            <td className="px-8 py-6">
                              <span
                                className={`text-xs font-bold uppercase tracking-widest ${item.hasWavvault ? 'text-neon-lime' : 'text-white/20'}`}
                              >
                            ...
                            <td className="px-8 py-6">
                              ...
                              <span className="text-[10px] text-white/40 mt-1 block font-mono">
                                {item.onboardingProgress || 0}% Complete
                              </span>
                            </td>
                            <td className="px-8 py-6">
                              <span className="text-xs font-medium text-white/40">
                                {item.metadata?.lastSignInTime
                                  ? new Date(item.metadata.lastSignInTime).toLocaleString()
                                  : 'Never'}
                              </span>
                            </td>
                          </>
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, hasWavvault: e.target.checked })
                        }
                      />
                      <label
                        htmlFor="hasWavvault"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, onboardingProgress: parseInt(e.target.value) })
                        }
                      />
                    </div>
                  </>
