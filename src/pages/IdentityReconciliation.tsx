...
export const IdentityReconciliation: React.FC = () => {
  ...
  const handleDeleteOrphan = async (dashboardId: string) => {
    if (!confirm(`Are you sure you want to PERMANENTLY DELETE this orphaned dashboard? This cannot be undone.`)) return;
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/40 text-sm mt-1">Diagnose and fix disconnected user dashboards</p>
        </div>
        <button 
          onClick={fetchData}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          { label: 'Total Users', value: data?.stats.totalUsers, icon: User, color: 'text-blue-400' },
          { label: 'Total Dashboards', value: data?.stats.totalDashboards, icon: LayoutDashboard, color: 'text-purple-400' },
          { label: 'Orphaned Dashboards', value: data?.stats.orphans, icon: AlertTriangle, color: 'text-neon-magenta' },
          { label: 'Users Missing Dashboards', value: data?.stats.missing, icon: UserPlus, color: 'text-yellow-400' },
        ].map((stat, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
          <div className="p-6 border-b border-white/10 bg-white/5">
            ...
            <p className="text-xs text-white/40 mt-1">Dashboards whose userId does not match any existing user UID</p>
          </div>
          <div className="divide-y divide-white/10 max-h-[500px] overflow-y-auto">
            {data?.orphanedDashboards.length === 0 ? (
              <div className="p-12 text-center text-white/20 italic">No orphaned dashboards found</div>
            ) : (
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right flex items-center gap-4">
                      <div>
                        ...
                        <p className="text-xs">{dash.updatedAt ? new Date(dash.updatedAt).toLocaleDateString() : 'N/A'}</p>
                      </div>
                      <button 
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
          <div className="p-6 border-b border-white/10 bg-white/5">
            ...
            <p className="text-xs text-white/40 mt-1">Users who do not have a dashboard document matching their UID</p>
          </div>
          <div className="divide-y divide-white/10 max-h-[500px] overflow-y-auto">
            {data?.usersWithoutDashboards.length === 0 ? (
              <div className="p-12 text-center text-white/20 italic">No users missing dashboards</div>
            ) : (
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="relative group">
                        ...
                        <div className="absolute right-0 top-full mt-2 w-64 bg-gray-900 border border-white/10 rounded-xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 p-2">
                          <p className="text-[10px] text-white/40 px-2 mb-2">Select orphan to link to this user:</p>
                          <div className="max-h-40 overflow-y-auto space-y-1">
                            {data?.orphanedDashboards.map(orphan => (
                              ...
                              >
                                <span className="truncate mr-2">{orphan.sparkwavvId || orphan.id}</span>
      ...
      <div className="bg-neon-cyan/5 border border-neon-cyan/20 p-6 rounded-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm">
          <div className="space-y-2">
            ...
            <p className="text-white/60 leading-relaxed">The system automatically re-links dashboards if a user logs in with a new UID but the same SparkwavvID. Manual re-linking is only needed for legacy disconnects.</p>
          </div>
          <div className="space-y-2">
            <p className="font-bold text-white/80">Orphaned Dashboards</p>
            <p className="text-white/60 leading-relaxed">These usually occur when a user's account is deleted but their dashboard remains. You can link these to a new user record or delete them via the database.</p>
          </div>
          <div className="space-y-2">
            <p className="font-bold text-white/80">Missing Dashboards</p>
            <p className="text-white/60 leading-relaxed">New users get a dashboard automatically. If a user is missing one, they will see a "Create Dashboard" prompt on their first login.</p>
          </div>
        </div>
      </div>
    </div>
