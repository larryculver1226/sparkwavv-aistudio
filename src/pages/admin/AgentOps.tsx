...
import { HelpCircle, Info, Target, Check, X, TerminalSquare, Activity, Settings2, ShieldAlert } from 'lucide-react';
...
export const AgentOps: React.FC = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto p-8 text-white space-y-12">
      ...
      {/* TESTING & MONITORING GUIDANCE SECTION */}
      <section className="bg-gray-900 p-8 rounded-[2rem] border border-gray-800 space-y-6 shadow-2xl">
        ...
        <div className="space-y-4 text-sm text-gray-300">
          <p>
            Sparkwavv dynamically runs Genkit client-side. Interact with Skylar in your app to capture live input/output JSON traces.
          </p>
          
          {genkitTraces.length === 0 ? (
            <div className="h-40 flex items-center justify-center border-2 border-dashed border-gray-700 rounded-xl bg-gray-800/50">
              <span className="text-gray-500 font-medium">Waiting for Genkit activity... Talk to Skylar to generate a trace.</span>
            </div>
          ) : (
              ...
              <div className="lg:col-span-1 space-y-2 max-h-96 overflow-y-auto pr-2">
                {genkitTraces.map((trace, idx) => (
                  ...
                  >
                    ...
                    <div className="text-gray-500 truncate">{new Date(trace.timestamp).toLocaleTimeString()}</div>
                    {trace.executedActions?.length > 0 && (
                      <div className="mt-2 flex gap-1 flex-wrap">
                        {trace.executedActions.map((a: any, i: number) => (
                          <span key={i} className="bg-purple-900/40 text-purple-300 px-1.5 py-0.5 rounded text-[10px]">
                            {a.action}
                          </span>
                        ))}
                      </div>
        ...
        {editStage && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="space-y-4 p-6 bg-gray-800/50 rounded-2xl border border-gray-700">
                <div className="flex justify-between items-center">
                  <label className="block text-sm font-medium text-gray-300">Page Widgets (Dynamic Layout)</label>
                  <button 
                    onClick={() => {
                      const newWidget = {
                        id: `w-${Date.now()}`,
                ...
                <div className="space-y-3">
                  {(editStage.uiConfig.widgets || []).sort((a, b) => a.order - b.order).map((widget, idx) => (
                    <div key={widget.id} className="flex flex-col md:flex-row gap-3 bg-gray-900 p-4 rounded-xl border border-gray-700 items-start md:items-center">
                      <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-3 w-full">
                        <select 
                          ...
                          onChange={(e) => {
                            ...
                            setEditStage({ ...editStage, uiConfig: { ...editStage.uiConfig, widgets: newWidgets } });
                          }}
                      ...
                      {widget.type === 'CustomMarkdown' && (
                        <input 
                          ...
                          onChange={(e) => {
                            const newWidgets = [...(editStage.uiConfig.widgets || [])];
                            newWidgets[idx].props = { ...newWidgets[idx].props, content: e.target.value };
                            setEditStage({ ...editStage, uiConfig: { ...editStage.uiConfig, widgets: newWidgets } });
                          }}
                      ...
                      <button 
                        onClick={() => {
                          const newWidgets = editStage.uiConfig.widgets?.filter(w => w.id !== widget.id);
                          setEditStage({ ...editStage, uiConfig: { ...editStage.uiConfig, widgets: newWidgets } });
                        }}
                  ...
                  {(!editStage.uiConfig.widgets || editStage.uiConfig.widgets.length === 0) && (
                    <div className="text-sm text-gray-500 text-center py-4">No widgets configured for this stage.</div>
                  )}
                </div>
              </div>
