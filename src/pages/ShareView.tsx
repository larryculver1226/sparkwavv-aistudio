...
import { Loader2, Share2, Download, ExternalLink, ChevronRight, ChevronLeft, LayoutGrid, List } from 'lucide-react';
...
const ShareView: React.FC = () => {
  ...
  const [shareData, setShareData] = useState<{ asset: SynthesizedAsset; brandingPersona: string; userName: string } | null>(null);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center text-white">
        ...
        <p className="mt-4 font-mono text-sm tracking-widest uppercase opacity-50">Decrypting Asset...</p>
      </div>
    );
  }
  ...
  if (error || !shareData) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center text-white p-4 text-center">
        ...
        <p className="text-gray-400 max-w-md">{error || 'This share link is no longer active or is invalid.'}</p>
        <button 
  ...
  return (
    <div className={`min-h-screen ${theme.bg} ${theme.text} ${theme.font} transition-colors duration-1000`}>
      {/* Header / Navigation */}
      <header className={`fixed top-0 left-0 right-0 z-50 p-6 flex justify-between items-center backdrop-blur-md border-b ${theme.border}`}>
        <div className="flex items-center gap-4">
          <div className={`w-10 h-10 rounded-full ${theme.accentBg} flex items-center justify-center text-white font-bold`}>
            {userName.charAt(0)}
          </div>
          <div>
            ...
            >
              ...
              <section className="min-h-[60vh] flex flex-col justify-center">
                ...
                <p className="text-xl md:text-2xl opacity-70 max-w-2xl leading-relaxed">
                  {asset.content.summary || "A synthesized view of professional DNA, strengths, and career milestones."}
                </p>
              </section>
              ...
              <section className="grid md:grid-cols-2 gap-12 items-center">
                ...
                <div className={`aspect-square rounded-full ${theme.border} border-dashed flex items-center justify-center p-12`}>
                  <div className={`w-full h-full rounded-full ${theme.accentBg} opacity-10 animate-pulse`} />
                  <div className="absolute text-center">
                    <span className="text-4xl font-bold block">88%</span>
                    <span className="text-[10px] uppercase tracking-widest opacity-50">Role Alignment</span>
                  </div>
                </div>
              </section>
              ...
              <section className="space-y-12">
                ...
                <div className="grid gap-8">
                  {asset.content.stories?.map((story: any, i: number) => (
                    ...
                    >
                      <span className={`text-[10px] uppercase tracking-widest mb-4 block ${theme.accent}`}>Story 0{i+1}</span>
            ...
            >
              ...
              <div className={`md:col-span-2 p-8 rounded-3xl ${theme.card}`}>
                ...
                <div className="aspect-video bg-black/5 rounded-2xl flex items-center justify-center border border-dashed border-black/10">
                  <p className="text-sm opacity-50 italic">Interactive Knowledge Graph Visualization</p>
                </div>
              </div>

              <div className="space-y-6">
                <div className={`p-6 rounded-3xl ${theme.card}`}>
                  <h4 className="text-sm font-bold uppercase tracking-widest mb-4 opacity-50">Identity</h4>
                  <p className="text-lg leading-snug">{asset.content.identity}</p>
                </div>
                <div className={`p-6 rounded-3xl ${theme.card}`}>
                  <h4 className="text-sm font-bold uppercase tracking-widest mb-4 opacity-50">Quick Stats</h4>
                  <div className="space-y-4">
      ...
      <footer className={`fixed bottom-0 left-0 right-0 p-6 flex justify-center gap-4 z-50`}>
        <div className={`flex items-center gap-2 p-2 rounded-full backdrop-blur-xl border ${theme.border} bg-white/10`}>
          <button className={`flex items-center gap-2 px-6 py-2 rounded-full ${theme.accentBg} text-white font-bold text-sm hover:scale-105 transition-transform`}>
