...
import { runJourneyStageFlow, analyzeWavvaultArtifactFlow } from './backend/services/genkitService';
  ...
  try {
    ...
    if (sparkwavvId) {
      ...
      if (!snapshot.empty) {
        ...
        console.log(
          `[IDENTITY] Self-healing: Re-anchoring dashboard ${oldUserId} to new UID ${userId} via sparkwavvId ${sparkwavvId}`
        );
...
function calculateDynamicScores(dashboard: any) {
  ...
  const profileCompleteness =
    (dashboard.resumeStatus?.includes('Completed') || dashboard.resumeStatus?.includes('Optimized')
      ? 0.5
      : 0.2) +
    (dashboard.careerProfileStatus?.includes('Optimized') ||
    dashboard.careerProfileStatus?.includes('Completed')
      ? 0.5
      : 0.2);
  ...
  let careerHappiness = Math.round(
    milestoneProgress * 40 + stageProgress * 40 + profileCompleteness * 20
  );
  ...
  if (dashboard.discoveryProgress === 'Dive-In') {
    ...
    const diveInProgress =
      diveInMilestones.length > 0 ? completedDiveIn / diveInMilestones.length : 0;
  ...
  const marketResonance =
    jobMatches.length > 0
      ? Math.round(
          jobMatches.reduce((acc: number, j: any) => acc + j.matchScore, 0) / jobMatches.length
        )
      : 0;
...
async function startServer() {
      ...
      try {
        ...
        if (db) {
          ...
          if (userDoc.exists) {
            ...
            const tenantId =
              typeof rawTenantId === 'string' ? rawTenantId : rawTenantId?.tenantId || 'sparkwavv';
  ...
  const setUserRole = async (uid: string, role: string, tenantId: string = 'sparkwavv') => {
    try {
      ...
      if (
        [ROLES.ADMIN, ROLES.SUPER_ADMIN, ROLES.EDITOR, ROLES.VIEWER, ROLES.OPERATOR].includes(
          role as any
        )
      ) {
  ...
  const requireRole = (roles: string[], requiredEntryPoint?: 'admin' | 'operations') => {
    return async (req: express.Request, res: express.Response, next: express.NextFunction) => {
      ...
      if (!roles.includes(role) && role !== ROLES.SUPER_ADMIN) {
        console.warn(
          `[AUTH] Access denied for ${decodedToken.email}. Role: ${role}, Required: ${roles.join(', ')}`
        );
  ...
  if (isFirebaseAdminConfigured) {
    ...
    if (process.env.SENDGRID_API_KEY && process.env.SENDGRID_API_KEY.startsWith('SG.')) {
      ...
      console.warn(
        "[SENDGRID] API Key missing or invalid (must start with 'SG.'). Email features will be simulated."
      );
    }
      ...
      try {
        if (isFirebaseAdminConfigured && sparkwavvAdmin) {
          ...
          if (allowedRoles.includes(role)) {
            ...
            await logEvent('INFO', 'AUTH', `Admin login successful: ${decodedToken.email}`, {
              uid: decodedToken.uid,
              role,
              entryPoint,
            });
    ...
    app.get('/api/admin/profile', async (req, res) => {
      ...
      try {
        ...
        console.log(
          `[AUTH] Profile requested for ${decodedToken.email}. Role: ${role}, Tenant: ${tenantId}, Session EntryPoint: ${entryPoint}`
        );
      ...
      async (req, res) => {
        ...
        try {
          await db
            .collection('dashboards')
            .doc(userId)
            .set({ validationGateMode: mode }, { merge: true });
          res.json({ success: true });
        } catch (error) {
    ...
    app.post(
      ...
      async (req, res) => {
        ...
        if (role === ROLES.SUPER_ADMIN && actor.role !== ROLES.SUPER_ADMIN) {
          return res
            .status(403)
            .json({ error: 'Only Super Admins can assign the Super Admin role.' });
        }
        ...
        try {
          ...
          if (generationalPersona !== undefined)
            updateData.generationalPersona = generationalPersona;
          ...
          try {
            ...
            if (role || tenantId) {
              ...
              console.log(
                `[AUTH] Updated custom claims for ${uid}: role=${currentRole}, tenantId=${currentTenantId}`
              );
            }
          } catch (authError: any) {
          ...
          if (role && oldData && oldData.role !== role) {
            await logSecurityEvent(
              ...
              'ROLE_CHANGE',
              'WARNING',
              { uid, email: oldData.email },
              { oldRole: oldData.role, newRole: role },
              req.ip
            );
          } else {
            await logSecurityEvent(
              ...
              'USER_UPDATE',
              'INFO',
              { uid, email: oldData?.email || 'unknown' },
    ...
    app.get(
      '/api/partner/user-progress/:userId',
      requireRole([ROLES.USER, ROLES.ADMIN]),
      async (req, res) => {
        const partnerUid = (req as any).user.uid;
        ...
        try {
          ...
          if (!accessDoc.exists && (req as any).user.role !== ROLES.ADMIN) {
            return res
              .status(403)
              .json({ error: "You do not have access to this user's progress" });
          }
    ...
    app.get(
      '/api/admin/identity-reconciliation',
      requireRole([ROLES.SUPER_ADMIN, ROLES.ADMIN]),
      async (req, res) => {
        try {
          const db = getFirestoreDb();
        ...
        try {
          if (isFirebaseAdminConfigured && sparkwavvAdmin) {
            ...
            await logSecurityEvent(
              ...
              'INFO',
              { uid, email: 'unknown' },
              { details: `User ${disabled ? 'disabled' : 'enabled'}` },
              req.ip
            );
    ...
    app.post(
      ...
      async (req, res) => {
        ...
        try {
          ...
          if (
            targetEmail.toLowerCase() === 'larry.culver1226@gmail.com' ||
          ...
          try {
            ...
            console.warn(
              `[AUTH] User ${uid} not found in Auth during deletion, proceeding to delete from Firestore.`
            );
          }
    ...
    app.get(
      '/api/admin/programs',
      requireRole([ROLES.SUPER_ADMIN, ROLES.ADMIN, ROLES.EDITOR, ROLES.VIEWER, ROLES.OPERATOR]),
      async (req, res) => {
        try {
    ...
    app.get(
      ...
      requireRole([ROLES.SUPER_ADMIN, ROLES.ADMIN, ROLES.OPERATOR]),
      async (req, res) => {
        try {
    ...
    app.get(
      ...
      async (req, res) => {
        try {
          const authUsers =
            isFirebaseAdminConfigured && sparkwavvAdmin
              ? await sparkwavvAdmin.auth().listUsers(10)
              : { users: [] };
    ...
    app.get(
      ...
      async (req, res) => {
        try {
          ...
          res
            .status(500)
            .json({ error: 'Failed to fetch flagged content', details: error.message });
        }
      }
    );
    ...
    app.post(
      '/api/admin/flagged-content/:id/delete',
      requireRole([ROLES.SUPER_ADMIN, ROLES.ADMIN, ROLES.EDITOR]),
      async (req, res) => {
        const { id } = req.params;
        const actor = (req as any).user;
    ...
    app.get(
      ...
      async (req, res) => {
        try {
          ...
          const { tenantId: filterTenantId } = req.query;
          console.log(
            `[ADMIN] users-v2 called by ${(req as any).user?.email} (${(req as any).user?.uid}) with filterTenantId: ${filterTenantId}`
          );
          ...
          try {
            if (isFirebaseAdminConfigured && sparkwavvAdmin) {
              ...
              console.log(
                `[ADMIN] Found ${listUsersResult.users.length} raw users in Firebase Auth`
              );
              console.log(
                `[ADMIN] Auth User Emails:`,
                listUsersResult.users.map((u) => u.email).join(', ')
              );
              ...
              authUsers = await Promise.all(
                listUsersResult.users.map(async (u) => {
                  try {
                    ...
                    console.warn(
                      `[ADMIN] Error getting role for user ${u.uid}:`,
                      roleError.message
                    );
                    return {
                      uid: u.uid,
                ...
                return {
                  ...
                  tenantId:
                    typeof data.tenantId === 'string'
                      ? data.tenantId
                      : data.tenantId?.tenantId || 'sparkwavv',
                  sparkwavvId: data.sparkwavvId || null,
                };
              });
              console.log(
                `[ADMIN] Found ${firestoreUsers.length} users in Firestore collection 'users'`
              );
            } else {
            ...
            if (mergedUsersMap.has(u.uid)) {
              ...
              Object.keys(u).forEach((key) => {
                ...
                if (
                  ...
                  existing.displayName !== existing.uid
                ) {
                  return;
                }
          ...
          try {
            fs.writeFileSync(
              path.join(process.cwd(), 'users-v2-last-response.json'),
              JSON.stringify(
                {
          ...
          res
            .status(500)
            .json({ error: 'Failed to list users. Check if Firebase is fully initialized.' });
        }
      }
    );
      ...
      try {
        ...
        if (userDoc.exists) {
          ...
          if (!userData.sparkwavvId && (userData.role === ROLES.USER || !userData.role)) {
            console.log(
              `[AUTH] Self-healing: Generating missing sparkwavvId for ${decodedToken.uid}`
            );
          ...
          console.warn(
            `[AUTH] Profile NOT found for UID: ${decodedToken.uid} (${decodedToken.email}). Creating default profile.`
          );
          const sparkwavvId = await generateSparkwavvId(db);
          const newProfile = {
            uid: decodedToken.uid,
        ...
        if (error.code === 16 || error.message?.includes('UNAUTHENTICATED')) {
          console.error(
            'CRITICAL: Firestore Unauthenticated (Error 16). Check your Service Account credentials.'
          );
        }
    ...
    app.get('/api/user/wavvault-status', async (req, res) => {
      ...
      try {
        const decodedToken = await withTimeout(sparkwavvAdmin.auth().verifyIdToken(idToken), 15000);
        const wavvaultDoc = (await withTimeout(
          db.collection('wavvault').doc(decodedToken.uid).get(),
          15000
        )) as any;
    ...
    app.get(
      '/api/wavvault/user',
      requireRole([ROLES.USER, ROLES.ADMIN, ROLES.OPERATOR, ROLES.MENTOR, ROLES.AGENT]),
      async (req, res) => {
        ...
        if (userId !== authenticatedUser.uid && authenticatedUser.role !== ROLES.ADMIN) {
          return res
            .status(403)
            .json({ error: 'Forbidden: You can only view your own Wavvault data.' });
        }
    ...
    app.get(
      ...
      async (req, res) => {
        ...
        if (userId !== authenticatedUser.uid && authenticatedUser.role !== ROLES.ADMIN) {
          return res
            .status(403)
            .json({ error: 'Forbidden: You can only verify your own Wavvault data.' });
        }
    ...
    app.post(
      ...
      async (req, res) => {
        ...
        if (userId !== authenticatedUser.uid && authenticatedUser.role !== ROLES.ADMIN) {
          return res
            .status(403)
            .json({ error: 'Forbidden: You can only update your own Wavvault data.' });
        }
    ...
    app.post(
      ...
      async (req, res) => {
        ...
        if (userId !== authenticatedUser.uid && authenticatedUser.role !== ROLES.ADMIN) {
          return res
            .status(403)
            .json({ error: 'Forbidden: You can only upload artifacts for yourself.' });
        }
    ...
    app.post(
      ...
      async (req, res) => {
        ...
        if (userId !== authenticatedUser.uid && authenticatedUser.role !== ROLES.ADMIN) {
          return res
            .status(403)
            .json({ error: 'Forbidden: You can only save opportunities for yourself.' });
        }
    ...
    app.get(
      ...
      async (req, res) => {
        ...
        if (userId !== authenticatedUser.uid && authenticatedUser.role !== ROLES.ADMIN) {
          return res
            .status(403)
            .json({ error: 'Forbidden: You can only access your own chat history.' });
        }

        try {
          const wavvaultDoc = await db
            .collection('wavvault')
            .doc(userId as string)
            .get();
    ...
    app.post(
      ...
      async (req, res) => {
        ...
        if (!userId || !history)
          return res.status(400).json({ error: 'UserId and history are required' });

        if (userId !== authenticatedUser.uid && authenticatedUser.role !== ROLES.ADMIN) {
          return res
            .status(403)
            .json({ error: 'Forbidden: You can only update your own chat history.' });
        }
        ...
        try {
          await db
            .collection('wavvault')
            .doc(userId)
            .set({ chatHistory: history }, { merge: true });

          // Asynchronously extract intel from the last interaction if there's enough context
          // Try to trigger every 5 user messages, or if it's explicitly asked
          ...
          if (userMessages.length > 0 && userMessages.length % 3 === 0) {
            ...
            // Fire and forget
            analyzeWavvaultArtifactFlow({
              userId,
              content: recentMessages,
              type: 'chat_interaction',
            })
              .then(async (metadata) => {
                if (metadata && metadata.title && metadata.title !== 'Synthesis Error') {
                  try {
                    await writeArtifact({
                      userId,
                      type: 'intel',
                      title: `Interaction Intel: ${metadata.title}`,
                      content: recentMessages,
                      metadata: {
                    ...
                    console.error('Failed to write intel artifact:', e);
                  }
                }
              })
              .catch((e) => console.error('Intel extraction flow failed:', e));
          }
    ...
    app.post(
      ...
      async (req, res) => {
        ...
        try {
          ...
          const metadata = await analyzeWavvaultArtifactFlow({
            userId,
            content,
            type,
          });
          ...
          console.error('Error analyzing Wavvault artifact:', error);
          res.status(500).json({ error: error.message });
        }
      }
    );
    ...
    app.get('/api/shares/public/:shareId', async (req, res) => {
      ...
      try {
        ...
        res.json({
          asset: assetData,
          brandingPersona:
            shareData.brandingPersona || userData?.brandPersona || 'Left Brain (Kick/Yin)',
    ...
    app.post('/api/feedback', async (req, res) => {
      try {
        const { issueType, description, stepsToReproduce, attachmentUrl, url, browserInfo } =
          req.body;
        ...
        const docRef = (await withTimeout(
          db.collection('feedback_issues').add(feedbackDoc),
          10000
        )) as any;
    ...
    app.get(
      '/api/admin/feedback',
      requireRole([ROLES.SUPER_ADMIN, ROLES.ADMIN, ROLES.OPERATOR], 'admin'),
      async (req, res) => {
        try {
          const db = getFirestoreDb();
          if (!db) throw new Error('Firestore not initialized');
      ...
      async (req, res) => {
        ...
        try {
          ...
          if (!results || results.length === 0) {
            ...
            const fallbackResults = await searchSimilarWavvaults(
              query,
    ...
    app.post(
      ...
      async (req, res) => {
        ...
        if (userId !== authenticatedUser.uid && authenticatedUser.role !== ROLES.ADMIN) {
          return res
            .status(403)
            .json({ error: 'Forbidden: You can only execute actions for your own account.' });
        }
          ...
          if (action === 'update_dashboard') {
            ...
            if (field === 'discoveryProgress') {
              await db
                .collection('users')
                .doc(userId)
                .set({ journeyStage: value }, { merge: true });
            }
            ...
            await logSecurityEvent(
              ...
              'AI_DASHBOARD_UPDATE',
              'INFO',
              { uid: userId, email: authenticatedUser.email },
            ...
            if (strategicFields.includes(field)) {
              await notifyRPP(
                userId,
                `Major Career Shift Detected: ${field} updated to ${value}`,
                reasoning
              );
            }
    ...
    app.post(
      ...
      async (req, res) => {
        try {
          ...
          if (
            authenticatedUser.role !== ROLES.GUEST &&
            authenticatedUser.uid !== userId &&
            ![ROLES.ADMIN, ROLES.SUPER_ADMIN, ROLES.MENTOR, ROLES.OPERATOR, ROLES.AGENT].some((r) =>
              (authenticatedUser.roles || []).includes(r)
            )
          ) {
      ...
      async (req, res) => {
        ...
        try {
          for (const feed of RSS_FEEDS) {
            ...
            const topItems = (Array.isArray(items) ? items : [items])
              .slice(0, 5)
              .map((item: any) => ({
                title: item.title || item.title?.['#text'] || 'No Title',
                snippet:
                  item.description || item.summary || item.content?.['#text'] || 'No Snippet',
    ...
    app.post(
      '/api/user/mentor-note',
      requireRole([ROLES.ADMIN, ROLES.MENTOR, ROLES.OPERATOR]),
      async (req, res) => {
    ...
    app.post(
      '/api/skylar/market-intelligence',
      requireRole([ROLES.USER, ROLES.ADMIN]),
      async (req, res) => {
        ...
        try {
          ...
          if (industry.toLowerCase() === 'finance') {
            sectorInsight = await vertexService.getFinanceInsight(
              `Analyze the current market for ${role || 'general finance roles'}`,
              `Industry: Finance, Role: ${role || 'N/A'}`
            );
          } else if (industry.toLowerCase() === 'tech') {
            sectorInsight = await vertexService.getTechInsight(
              `Analyze the current market for ${role || 'general tech roles'}`,
              `Industry: Tech, Role: ${role || 'N/A'}`
            );
          } else if (industry.toLowerCase() === 'healthcare') {
            sectorInsight = await vertexService.getHealthcareInsight(
              `Analyze the current market for ${role || 'general healthcare roles'}`,
              `Industry: Healthcare, Role: ${role || 'N/A'}`
            );
          }
          ...
          res.json({
            ...
            cache: relevantCache,
            message:
              relevantCache.length > 0 || sectorInsight
                ? 'Found relevant intelligence.'
                : 'No relevant data found. Suggesting live search.',
          });
        } catch (error: any) {
          res.status(500).json({ error: error.message });
        }
     ...
     */
    app.get(
      '/api/skylar/tuning/status/:jobId',
      requireRole([ROLES.ADMIN, ROLES.SUPER_ADMIN]),
      async (req, res) => {
    ...
    app.post(
      '/api/skylar/patterns',
      requireRole([ROLES.USER, ROLES.ADMIN, ROLES.OPERATOR]),
      async (req, res) => {
        const { query } = req.body;
        const authenticatedUser = (req as any).user;
    ...
    app.post('/api/skylar/healthcare', requireRole([ROLES.USER, ROLES.ADMIN]), async (req, res) => {
      ...
      try {
        ...
        if (
          userData?.specializedSector !== 'Healthcare' &&
          authenticatedUser.role !== ROLES.SUPER_ADMIN
        ) {
          return res
            .status(403)
            .json({ error: 'MedLM access is restricted to the Healthcare sector.' });
        }
    ...
    app.post('/api/skylar/coaching', requireRole([ROLES.USER, ROLES.ADMIN]), async (req, res) => {
      ...
      try {
        ...
        res
          .status(500)
          .json({ error: error.message || 'Failed to get strategic coaching advice.' });
      }
    });
     ...
     */
    app.post(
      '/api/admin/vertex/generate-synthetic',
      requireRole([ROLES.ADMIN, ROLES.SUPER_ADMIN]),
      async (req, res) => {
        ...
        try {
          const trainingSet = await methodologyGenerator.generateTrainingSet(
    ...
    app.get(
      '/api/admin/vertex/synthetic-data',
      requireRole([ROLES.ADMIN, ROLES.SUPER_ADMIN]),
      async (req, res) => {
        try {
          const filePath = path.join(__dirname, 'synthetic-training-data.jsonl');
    ...
    app.post(
      '/api/admin/vertex/upload-to-gcs',
      requireRole([ROLES.ADMIN, ROLES.SUPER_ADMIN]),
      async (req, res) => {
        try {
          const filePath = path.join(__dirname, 'synthetic-training-data.jsonl');
          if (!fs.existsSync(filePath)) {
            return res
              .status(404)
              .json({ error: 'Synthetic data file not found. Please generate it first.' });
          }
  ...
  app.get('/api/user/dashboard', async (req, res) => {
    ...
    try {
      ...
      if (requestedUserId && requestedUserId !== userId && requestedUserId !== userSparkwavvId) {
        ...
        if (isAuthorized) {
          console.log(
            `🛡️ [Dashboard] ${userRole} ${userId} accessing dashboard for ${requestedUserId}`
          );
          userId = requestedUserId;

          // Re-fetch target user's sparkwavvId for dashboard lookup
          try {
            const targetDoc = (await withTimeout(
          ...
          console.warn(
            `🚫 [Dashboard] Unauthorized access attempt by ${userId} to ${requestedUserId}`
          );
          return res.status(403).json({ error: 'Forbidden' });
        }
      }
  ...
  app.post('/api/brand/synthesize', requireRole([ROLES.USER, ROLES.ADMIN]), async (req, res) => {
    ...
    try {
      ...
      await db
        .collection('dashboards')
        .doc(userId)
        .set({ brandSecretId: secretId }, { merge: true });
  ...
  app.post(
    ...
    requireRole([ROLES.MENTOR, ROLES.ADMIN]),
    async (req, res) => {
      try {
        const db = getFirestoreDb();
        if (!db) return res.status(503).json({ error: 'Database not available' });
        ...
        if (!accessDoc.exists || !accessDoc.data()?.permissions.includes('propose')) {
          return res
            .status(403)
            .json({ error: 'You do not have permission to propose changes for this user.' });
        }
  ...
  app.get('/api/user/suggestions', async (req, res) => {
    try {
      ...
      const snapshot = await db
        .collection('partner_suggestions')
  ...
  app.post(
    '/api/tenant/:id/settings',
    requireRole([ROLES.ADMIN, ROLES.SUPER_ADMIN, ROLES.MENTOR]),
    async (req, res) => {
      try {
        const user = (req as any).user;
  ...
  app.get('/api/partner/clients', requireRole([ROLES.MENTOR, ROLES.ADMIN]), async (req, res) => {
    try {
      ...
      const snapshot = await db.collection('users').where('tenantId', '==', partner.tenantId).get();
  ...
  app.get('/api/auth/status', async (req, res) => {
    ...
    res.json({
      ...
      projectId:
        process.env.FIREBASE_PROJECT_ID || process.env.VITE_FIREBASE_PROJECT_ID || 'not set',
    });
  });
  ...
  app.get(
    '/api/admin/stats',
    requireRole([ROLES.SUPER_ADMIN, ROLES.ADMIN, ROLES.EDITOR, ROLES.VIEWER]),
    async (req, res) => {
      let userStats = {
        total: 0,
        active: 0,
      ...
      if (isFirebaseAdminConfigured && sparkwavvAdmin) {
        try {
          ...
          const allUserIds = new Set([...authUsers.map((u) => u.uid)]);
          ...
          const newToday = authUsers.filter(
            (u) => u.metadata.creationTime && new Date(u.metadata.creationTime) >= today
          ).length;
  ...
  if (isFirebaseAdminConfigured && sparkwavvAdmin) {
    ...
    try {
      ...
      if (db) {
        ...
        if (targetUser) {
          console.log(
            '[DEBUG] Found target user in Firestore:',
            JSON.stringify(targetUser, null, 2)
          );

          // Check for dashboard
          const dashboardSnapshot = (await withTimeout(
            db.collection('dashboards').where('userId', '==', targetUser.uid).get(),
            15000
          )) as any;
          if (!dashboardSnapshot.empty) {
            console.log(
              `[DEBUG] Found ${dashboardSnapshot.size} dashboards for user ${targetUser.uid}`
            );
      ...
      fs.writeFileSync(
        path.join(process.cwd(), 'users-debug.json'),
